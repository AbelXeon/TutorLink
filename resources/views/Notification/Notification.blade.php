...
<div class="max-w-3xl mx-auto">
    <!-- Header with heading, dynamic dashboard link, and Mark All Read Action Button -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-200 pb-6 mb-8 gap-4">
        ...
        <div class="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
            <!-- UPDATED: Dynamic Back to Dashboard Arrow Link -->
            @if(strtolower(Auth::user()->role?->role_type) === 'teacher')
                <a href="{{ route('tutor.dashboard') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-500 flex items-center gap-1 transition select-none">
                    &larr; Back to Dashboard
                </a>
            @else
                <a href="{{ route('student.dashboard') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-500 flex items-center gap-1 transition select-none">
                    &larr; Back to Dashboard
                </a>
            @endif

            @if($notifications->where('read_at', false)->count() > 0)
                <!-- SECURE POST ACTION FORM -->
                <form action="{{ route('notifications.read.all') }}" method="POST">
                    @csrf
                    <button type="submit" class="inline-flex justify-center py-2 px-4 border border-indigo-600 rounded-md shadow-sm text-xs font-bold text-indigo-600 bg-white hover:bg-indigo-50 transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Mark All as Read
                    </button>
                </form>
            @endif
