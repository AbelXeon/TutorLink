        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="flex items-center space-x-6">
                   ...
                   <!-- Notification Icon with aligned badge -->
                    @auth
                        @php
                            $unreadCount = \App\Models\Notification::where('user_id', Auth::id())->where('read_at', false)->count();
                        @endphp
                    @endauth
                    <a href="{{ route('notifications.index') }}" class="text-gray-500 hover:text-indigo-600 transition relative inline-flex items-center justify-center p-1 rounded-full" title="Notifications">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                        </svg>
                        @if(isset($unreadCount) && $unreadCount > 0)
                            <span class="absolute top-0 right-0 block h-4 w-4 rounded-full bg-red-500 text-[9px] font-bold text-white text-center leading-4 shadow-sm">
                                {{ $unreadCount }}
                            </span>
                        @endif
                    </a>
